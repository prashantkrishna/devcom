class WorkExperience < ApplicationRecord
  ...
  validates :employment_type, presence: true, inclusion:  {in: EMPLOYMENT_TYPE, message: 'this is not a valid employment_type'}
  validates :location_type, presence: true, inclusion: {in: LOCATION_TYPE, message: 'this is not a valid location_type'}
  ...
  def presence_of_end_date
    if end_date.nil? && !currently_working_here
      errors.add(:end_date, 'End date must be if you are not currently working in this company')
    end
  end
  ...
  def end_date_greater_than_start_date
   ...
   def company_with_empolyment_type
